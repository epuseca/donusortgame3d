...
public class AudioController : Singleton<AudioController>
{
    ...
    public void PlaySound(AudioClip[] clips, AudioSource aus = null)
    {
        ...
        if (clips != null && clips.Length > 0 && aus && !aus.mute)
        {
    ...
    {
        if (musicAus && musicAus.isPlaying)
        {
            musicAus.Stop();
        }
        else
    ...
    public void StopPlaySfx()
    {
        if (sfxAus)
        {
            if (sfxAus.mute)
            {
                sfxAus.mute = false; // Unmute to allow sound effects to play
                sfxState = true;
            }
            else
            {
                sfxAus.Stop(); // Stop any currently playing sound effects
                sfxAus.mute = true; // Mute the AudioSource to prevent further playback
                sfxState = false;
            }
        }
    }
    ...
    {
        return musicAus.isPlaying;
    }
    public bool IsStateSfx()
    {
        return sfxState;
    }
}
